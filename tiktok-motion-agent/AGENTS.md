...
- size: `2160x3840`
- quality: `medium` by default. Avoid `high` unless Naufal explicitly asks; high quality with 3 references and 2160x3840 is heavier and has caused OpenAI image aborts.
- outputFormat: `jpeg`
- openai.outputCompression: `88`
...
```
- It must be readable, non-empty, non-blank, exact 9:16, and at least 1080x1920. Preferred output remains `2160x3840` JPEG, now with medium/low quality to reduce OpenAI aborts.
- If it comes back `1024x1536` / 2:3, too small, corrupt, blank, or wrong format, regenerate before continuing unless Naufal explicitly approves changing the validation/output aspect policy.
- Then run a visual sanity check with the image tool before `complete`. The generated reference MUST show exactly one person only, no duplicated/extra subject, no side-by-side/comparison/collage/mirror duplicate, no product-reference model copied as another person, no severe anatomy/face/hand issue, modest coverage, and the selected master scene/framing/background preserved.
- If the visual sanity check fails, DO NOT run `complete` and DO NOT submit to FigmaWave/Weavy. Regenerate once with a stricter prompt if the issue is likely fixable. If it still fails, set the job `REJECTED` with a short note like `generated reference visual sanity failed: <reason>` and stop.
- Do NOT run or require the semantic product-match/reference-image checker before `complete`, and do not regenerate solely because of semantic product-match concerns. The prompt must still ask the image model to match the product closely; this rule only disables the checker/regeneration loop as a blocker. The visual sanity check is only for hard structural issues: duplicate people, wrong background/scene, severe artifacts, modesty failure, corrupt/wrong-size image.
