...
def extract_product_video_frames(video_path: Path, job_dir: Path, count: int = 2):
    """Extract high-quality stills from the product TikTok video for try-on refs."""
    duration = video_duration_seconds(video_path)
    if duration and duration > 2:
        points = [duration * 0.35, duration * 0.65]
    else:
        points = [1.0, 3.0]
    frames = []
    for idx, seconds in enumerate(points[:count], start=1):
        out = job_dir / f"product_video_frame_{idx}.jpg"
        cmd = [
            "ffmpeg",
            "-y",
            "-hide_banner",
            "-loglevel", "error",
            "-ss", f"{max(0, seconds):.3f}",
            "-i", str(video_path),
            "-frames:v", "1",
            "-q:v", "2",
            str(out),
        ...
        if proc.returncode != 0 or not out.exists() or out.stat().st_size < 5 * 1024:
            detail = (proc.stderr or proc.stdout or "").strip()[-500:]
            raise RuntimeError(f"Could not extract product video frame {idx}: {detail}")
        frames.append(out)
    if not frames:
        raise RuntimeError("Could not extract any product video frames")
    return frames
